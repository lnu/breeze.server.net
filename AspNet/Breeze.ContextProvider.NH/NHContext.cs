...
namespace Breeze.ContextProvider.NH {
  public class NHContext : Breeze.ContextProvider.ContextProvider, IDisposable {
    ...
    protected override void SaveChangesCore(SaveWorkState saveWorkState) {
      ...
      // for transactionscope http://nhibernate.info/doc/nhibernate-reference/transactions.html#transactions-scopes
      // no need to start an nhibernate transaction if inside transactionscope. Everything will be handled inside nhibernate(from 5.0.0).
      try {
    ...
    {
        // entities relations are removed in AfterSaveEntities so we can stil work with associations in AfterSaveEntitiesDelegate or AfterSaveEntities override
